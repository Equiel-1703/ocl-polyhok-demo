...
OCLPolyHok.defmodule RayMarching do
	...
	defd scene_sdf(p_x, p_y, p_z) do
		# For testing, I will just add a single sphere in the center of the scene
		# with radius 1
		s1_x = 0.0
		s1_y = 0.0
		s1_z = -1.0
	...
	defd ray_march(ray_origin_x, ray_origin_y, ray_origin_z, ray_dir_x, ray_dir_y, ray_dir_z) do
		total_distance = 0.0
		...
		for i in range(0, max_steps) do
			...
			dist_to_closest = scene_sdf(current_x, current_y, current_z)
	...
	defk render_kernel(img_array, dim_x, dim_y) do
		...
		if tid_x < dim_x && tid_y < dim_y do
			...
			dist = ray_march(cam_x, cam_y, cam_z, ray_dir_x, ray_dir_y, ray_dir_z)

			if dist > 0.0 do
				# We hit something, we will do a simple shading based on the distance traveled (closer objects are brighter)
				brightness = 1.0 - (dist)
				brightness = max(0.0, min(1.0, brightness)) # Clamp brightness to [0, 1]

				img_array[tid_y * dim_x + tid_x] = trunc(brightness * 255.0)
